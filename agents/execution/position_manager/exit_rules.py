...
청산 파라미터는 ``config/strategy_params.yaml``의 take_profit/stop_loss
에서만 읽는다. 이 파일은 live 모드에서 SHA-256 해시 잠금(§3.3) 대상이므로
**읽기 전용**으로만 접근한다.

**타임스톱(시간 기반 매도)은 완전히 제거되었다.** 매도는 오직 익절(TP1/TP2/트레일링)·
손절(기술적/하드)·EOD 강제청산(15:20)으로만 발동하며, "일정 시간 방향이 안 나오면
판다"는 시간 트리거는 더 이상 존재하지 않는다.
"""
...
class CounterEffect(str, Enum):
    ...
    NEUTRAL = "neutral"       # EOD 강제청산 → 카운터 미산입
...
class ExitParams:
    ...
    # tp1_pct/tp2_pct는 ATR을 알 수 없을 때의 폴백(범위 하단).
    # 1차 +3~5% / 50% 청산, 2차 +6~8% / 30% 청산.
    tp1_low: float = 0.03
    tp1_high: float = 0.05
    tp1_pct: float = 0.03
    ...
    trailing_pct: float = 0.015           # 고점 대비 -1.5% 이탈 시 잔량 전량(§5.3)
    hard_stop_pct: float = -0.03          # 최대 손절 -3%(§5.4)
...
def evaluate_exit(
    ...
    *,
    ...
    우선순위(먼저 도달한 쪽, §5.2 매도 타점):
      EOD 강제청산(15:20) → 하드 손절 → 기술적(진입캔들 저점 이탈) → 트레일링(TP1 후,
      고점 이탈) → TP1 → TP2 → HOLD

    **타임스톱(시간 기반 매도)은 완전히 제거되었다** — "일정 시간 방향이 안 나오면
    판다"는 시간 트리거는 존재하지 않는다. 매도는 오직 익절·손절·EOD로만 발동한다.

    **신호붕괴(거래량 소멸/MACD 데드 등)로 파는 '조급한·신호 없는 청산' 로직도
    제거되었다** — 손절은 오직 진입캔들 저점 이탈(기술적)·하드 손절로만, 익절은 ATR
