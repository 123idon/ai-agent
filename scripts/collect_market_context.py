...
매핑(룩어헤드 없음 — 전부 D 이전 세션):
  · 나스닥종합(^IXIC)/S&P500(^GSPC)/VIX(^VIX) = D **직전 미국 거래일** 종가(= D 새벽에 끝난 미국장, "전일밤 미국증시")
  · 코스피(^KS11)/코스닥(^KQ11) = D **직전 한국 거래일** 종가(= "전일 지수")
  · 원/달러 환율(KRW=X) = D **직전 거래 세션** 값(외환은 연속거래 → D 이전 마지막 일자)

KOSPI200 선물/야간선물은 yfinance 가 안정적인 티커를 제공하지 않아(KS11=F·KS200=F 모두 0행)
포함하지 않는다(품질 미달, 의도적 제외).
...
TICKERS = {
    ...
    "sp500": ("^GSPC", "S&P500", "직전 미국장 마감(전일밤)"),
    "vix": ("^VIX", "VIX 공포지수", "직전 미국장 마감(전일밤)"),
    "kospi": ("^KS11", "코스피", "직전 한국 거래일 종가"),
    "kosdaq": ("^KQ11", "코스닥", "직전 한국 거래일 종가"),
    "usdkrw": ("KRW=X", "원/달러 환율", "직전 거래 세션(전일 환율)"),
}
